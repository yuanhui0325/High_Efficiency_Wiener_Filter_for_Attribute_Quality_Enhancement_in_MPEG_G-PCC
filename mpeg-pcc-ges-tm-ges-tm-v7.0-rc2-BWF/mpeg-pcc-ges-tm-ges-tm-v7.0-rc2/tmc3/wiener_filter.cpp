  ...
  AttributeInterPredParams& attrInterPredParams)
{
  ...
  //ofstream txt;
  //txt.open("loot_r06_intra.txt", ios::app);
  ...
  if (recon_pc->hasColors()) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      //  f_pc.col(i), qp);    ////////////
      f_J[i] = cost_calculate(
        color_bits / 3. + 3 + 200, col_o_yuv.col(i), f_pc.col(i),
        qp); 
  ...
  AttributeInterPredParams& attrInterPredParams)
{
  ...
  //}
    ...
    if (yuv1[0] == 1) {
       for (int i = 0; i < K_wiener; ++i) {
            coef[i][0] = decoder.decode();
        }
    }
    if (yuv1[1] == 1) {
        for (int i = 0; i < K_wiener; ++i) {
            coef[i][1] = decoder.decode();
        }
    }
    if (yuv1[2] == 1) {
        for (int i = 0; i < K_wiener; ++i) {
            coef[i][2] = decoder.decode();
        }
    }
